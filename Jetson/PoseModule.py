# Сгенерировано LLM для переноса PoseModule
# под новые версии mediapipe
...
def _downloadFile(url: str, path: str) -> None:
    tmp_path = path + ".part"
...
def _normalizeIndex(value) -> int:
    if hasattr(value, "value"):
        value = value.value
    return int(value)


def _getPoseConnections():
    """
...
class PoseModule:
    def __init__(
        ...
        self.model_path = self._prepareModel(model_path, auto_download)
        self._lastTimestampMs = 0
        self._connections = _getPoseConnections()
    ...
    def _prepareModel(self, model_path, auto_download):
        """
        ...
        if auto_download and model_url:
            ...
            try:
                _downloadFile(model_url, model_path)
            except Exception as exc:
    ...
    def process(self, img, draw=True):
        ...
        if self.static_image_mode:
            result = self.landmarker.detect(mpImage)
        else:
            timestampMs = int(time.time() * 1000)

            if timestampMs <= self._lastTimestampMs:
                timestampMs = self._lastTimestampMs + 1

            self._lastTimestampMs = timestampMs
            result = self.landmarker.detect_for_video(mpImage, timestampMs)

        raw_landmarks = []

        if result is not None:
            poseLandmarks = getattr(result, "pose_landmarks", None)
            if poseLandmarks:
                raw_landmarks = poseLandmarks[0] or []

        self.results = _TaskResult(raw_landmarks)
        self.lms = self._getLandmarks() or []

        if draw and self.results.pose_landmarks:
            self._drawLandmarks(img, self.results.pose_landmarks.landmark)
    ...
    def _drawLandmarks(self, img, landmarks):
        h, w = img.shape[:2]
        points = []
        ...
        for connection in self._connections:
            if hasattr(connection, "start") and hasattr(connection, "end"):
                a = _normalizeIndex(connection.start)
                b = _normalizeIndex(connection.end)
            else:
                try:
                    a, b = connection
                    a = _normalizeIndex(a)
